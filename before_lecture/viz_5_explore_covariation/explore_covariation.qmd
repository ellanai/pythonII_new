...
```{python}
penguins_bm = penguins.groupby("year")["body_mass_g"].mean()

penguins_bm_df = penguins_bm.reset_index()

alt.Chart(penguins_bm_df).mark_bar().encode(
...
alt.Chart(penguins).mark_bar().encode(
    alt.Y('average(body_mass_g)')
)
```


```{python}
lt.Chart(penguins).mark_bar
