...
poly.bounds
# prints x line, x line, y line, y line (although exact order is not certain -- google it)
```

```{python}
print("Polygon Exterior: ", poly.exterior)
poly.exterior
# this is how to measure the perimeter -- extract exterior then apply length method (see below)
```
...
# defined line string to connect four points but not connecting the fourth to the first
```
...
```{python}
dps_point1 = Point(2,4)
dps_point2 = Point(3,7)
dps_point3 = Point(4,3)
dps_poly = Polygon([dps_point1, dps_point2, dps_point3])
dps_poly
```
...
```{python}
dps_point1 = Point(2,4)
dps_point2 = Point(2,6)
dps_point3 = Point(4,4)
dps_point4 = Point(4,6)
dps_poly = Polygon([dps_point1, dps_point2, dps_point4, dps_point3])
dps_poly
...
```{python}
dps_circle = dps_point1.buffer(3)
dps_circle
```
